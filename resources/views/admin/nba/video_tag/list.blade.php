                                    ...
                                    <div class="am-form-group">
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <a href="{{ url('nba/nba_video_tag/create')}}" type="button" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 添加视屏标签</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        	...
                                        	@foreach($videoTag as $k)
	                                            <tr class="gradeX">
	                                                ...
	                                                <td class="am-text-middle">
														@if($k->is_show === 0)
                                                       		 <a href="{{ url('nba/nba_video_tag/'.$k->id.'?is_show=0')}}" class="am-badge am-badge-danger am-text-sm">隐藏</a>
	                                                    @elseif($k->is_show == 1)
	                                                         <a href="{{ url('nba/nba_video_tag/'.$k->id.'?is_show=1')}}" class="am-badge am-badge-success am-text-sm">显示</a>
	                                                    @endif
	                                                </td>
	                                                <td class="am-text-middle">
	                                                    <div class="tpl-table-black-operation">
	                                                        <a href="{{ url('nba/nba_video_tag/'.$k->id.'/edit') }}">
